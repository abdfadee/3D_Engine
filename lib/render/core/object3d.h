...
public:
    ...
    void updateLocalMatrixIfNeeded() {
        ...
        localMatrix = glm::rotate(localMatrix,radians(rotationVector.z), vec3(0, 0, 1));
        localMatrix = glm::rotate(localMatrix, radians(rotationVector.y), vec3(0, 1, 0));
        localMatrix = glm::rotate(localMatrix, radians(rotationVector.x), vec3(1, 0, 0));
    ...
    virtual void link(mat4 parentMatrix) {
        updateLocalMatrixIfNeeded();
        worldMatrix = parentMatrix * localMatrix;
     
        for (auto* child : children)
            child->link(worldMatrix);
    }

    virtual void render(mat4 parentMatrix,bool materialize = true) {
        updateLocalMatrixIfNeeded();
        worldMatrix = parentMatrix * localMatrix;
